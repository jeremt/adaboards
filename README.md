...
- [ ] Mettre en place les routes d'authentification. En utilisant un JWT:
    - [ ] La route de login via `POST /api/auth/login`
    - [ ] La route d'inscription `POST /api/auth/register`
- [ ] CRUD de l'entité board: 
    - [ ] implémenter la route `GET /api/boards`
    - [ ] implémenter la route `POST /api/boards`
    - [ ] implémenter la route `PATCH /api/boards/{boardId}`
    - [ ] implémenter la route `DELETE /api/boards/{boardId}`
- [ ] CRUD de l'entité task:
    - [ ] Implémenter la route `GET /api/boards/{boardId}/task/`
    - [ ] Implémenter la route `POST /api/boards/{boardId}/task/`
    - [ ] Implémenter la route `PATCH /boards/{boardId}/task/{taskId}`
    - [ ] Implémenter la route `DELETE /api/boards/{boardId}/task/{taskId}`
- [ ] Protéger les routes pour faire en sorte d'appliquer la logique des droits de lecture ou d'écriture aux différents boards. Exemple: si un utilisateur sur un board n'a le droit de lecture uniquement, une requête `POST /api/boards/{boardId}/task/` devrait retourner une 401. De même, un utilisateur qui essaie d'acceder aux taches d'un board sur lequel il n'est pas membre via `GET /api/boards/{boardIdQuiN'estPasLeMien}` retournera aussi une 401.
